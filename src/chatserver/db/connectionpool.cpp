...
{
	FILE* pf = fopen("/project/wsm/server/src/chatserver/db/mysql.txt", "r");  //可执行文件在bin目录
	if (pf == nullptr)
	{
		LOG("mysql.txt file is not exits!");
		return false;
	}
...
void ConnectionPool::scannerConnectionTask()
{
	for (;;)
	{
		...
		//扫描整个队列, 释放多余的连接
		unique_lock<mutex> lock(_queueMutex);
		while (_connectionCnt > _initSize) // 这里要判断下队列是否为空  因为线程池里面放的是没有被用的连接
		{
