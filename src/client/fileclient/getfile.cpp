...
bool getfile::setlogfile(string strlogfile)
{
...
bool getfile::setxml(string strxml)
{
    this->strxml = strxml; 
...
// 登录业务。 
bool getfile::Login(const char *argv)    
{
...
// 把xml解析到参数starg结构
bool getfile::_xmltoarg(const char *strxmlbuffer)
{
...
// 文件下载的主函数。
void getfile::_tcpgetfiles()
{
    PActive.AddPInfo(starg.timeout,starg.pname);

    while (true)
    {
        logfile.Write("_tcpgetfiles while true\n");
        memset(strsendbuffer,0,sizeof(strsendbuffer));
        memset(strrecvbuffer,0,sizeof(strrecvbuffer));
...
// 接收文件的内容。
bool getfile::RecvFile(const int sockfd,const char *filename,const char *mtime,int filesize)
{
    // 生成临时文件名。
