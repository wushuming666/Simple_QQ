...
// 登录客户端的登录报文。
bool ClientLogin(CTcpServer* server);

// 接收文件主函数
void RecvFilesMain(void* server);

// 接收对端发送过来的文件内容。
bool RecvFile(const int sockfd, const char* filename, const char* mtime, const int filesize);

// 发送文件的主函数。
bool _tcpputfiles(CTcpServer TcpServer);
...
// 发送文件主函数
void SendFilesMain(CTcpServer* server);

// 向对端发送心跳报文
bool ActiveTest(CTcpServer* server);
...
{
    CTcpServer *TcpServer = (CTcpServer *) arg;
    // int flags = fcntl(TcpServer->m_connfd, F_GETFL);
    // if (flags == -1) perror("--wsm thmain TcpServer:");
    // TcpServer->m_connfd = 
    // 以下是子进程，负责与客户端通信。
    logfile.Write("3 客户端(%s)已连接。\n", TcpServer->GetIP());
...
// int flags = fcntl(TcpServer->m_connfd, F_GETFL);
// if (flags == -1) perror("3--wsm thmain TcpServer:");

    // 接收文件主函数
    if (starg.clienttype == 1) RecvFilesMain((void*)TcpServer);

    // 发送文件主函数
    if (starg.clienttype == 2) SendFilesMain(TcpServer);
}
...
logfile.Write("ok\n");
    ...
    epev.events = EPOLLIN;
    epev.data.fd = TcpServer.m_listenfd;
    epoll_ctl(epfd, EPOLL_CTL_ADD, TcpServer.m_listenfd, &epev);

    logfile.Write("TcpServer.m_listenfd = %d\n", TcpServer.m_listenfd);
    ...
    while (true)
    {
        ...
        for(int i = 0; i < ret; i++)
        {
            ...
            logfile.Write("curfd = %d\n", curfd);
            if(curfd == TcpServer.m_listenfd) {
                // 监听的文件描述符有数据达到，有客户端连接
                TcpServer.m_socklen = sizeof(struct sockaddr_in);
                TcpServer.m_connfd = accept(TcpServer.m_listenfd, (struct sockaddr *)&TcpServer.m_clientaddr, (socklen_t*)&TcpServer.m_socklen);
                logfile.Write("1 客户端(%s)已连接。\n", TcpServer.GetIP());
                epev.events = EPOLLIN | EPOLLONESHOT;
                epev.data.fd = TcpServer.m_connfd;
                epoll_ctl(epfd, EPOLL_CTL_ADD, TcpServer.m_connfd, &epev);
            }
            else
            {
                // 有数据到达，创建一个线程处理
                pthread_t thid=0;
                TcpServerArr[idx] = TcpServer;
                TcpServerArr[idx].m_connfd = curfd;
                logfile.Write("2 客户端(%s)已连接。\n", TcpServerArr[idx].GetIP());

    // int flags = fcntl(TcpServer.m_connfd, F_GETFL);
    // if (flags == -1) perror("thmain TcpServer:");

                if (pthread_create(&thid,NULL,thmain,&TcpServerArr[idx++])!=0)
                {
...
flags = fcntl(server->m_connfd, F_GETFL);
if (flags == -1) perror("2--wsm thmain TcpServer:");
    return true;
}
...
void RecvFilesMain(void* server)
{
    CTcpServer TcpServer = *(CTcpServer*)server;
    // int flags = fcntl(TcpServer.m_connfd, F_GETFL);
    // if (flags == -1) perror("------RecvFilesMain:");
    // int flags = fcntl(TcpServer.m_connfd, F_GETFD);
    // if (flags == -1) perror("RecvFilesMain:");
    logfile.Write("RecvFilesMain TcpServer.m_connfd = %d\n", TcpServer.m_connfd);
    while (true)
    {
cout << "true" << endl;
        memset(strRecvBuffer, 0, sizeof(strRecvBuffer));
...
void SendFilesMain(CTcpServer* server)
{
    ...
    {
        // 发送文件的主函数。
        if (_tcpputfiles(TcpServer) == false) { logfile.Write("_tcpputfiles() failed.\n"); return; }
        ...
        if (bcontinue == false)
        {
            ...
            if (ActiveTest(server) == false) break;
        }
    }
}
